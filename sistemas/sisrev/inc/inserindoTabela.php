...
$execSmart = oci_parse($connBpmgp, $emp);
oci_execute($execSmart);


while ($row = oci_fetch_array($execSmart,OCI_ASSOC)) {
  ...
  echo '<tr>
                  ...
                  <td><a href="editEmp.php?pg=' . $_GET["pg"] . '&tela=3&ID=' . $row["ID_EMPRESA"] . '" title="Editar" class="btn-primary btn-sm" ' . $usuarioFuncao . '><i class="bi bi-pencil"></i></a>
                            ...
                            <a href="../front/deletarEmp.php?ID=' . $row["ID_EMPRESA"] . '" title="Desativar" class="btn-danger btn-sm" ' . $usuarioFuncao . '><i class="bi bi-trash"></i></a>
