      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-body">
            ...
            $conexaoNew = oci_parse($connBpmgp, $queryCountNovo);
            oci_execute($conexaoNew);

            while ($countNew = oci_fetch_array($conexaoNew, OCI_ASSOC)) {
              $usuarioNew = $countNew['QUANTIDADE'];
            }

            oci_free_statement($conexaoNew);


            ?>
            <form class="row g-3" action="../inc/atualizandoGestor.php?pg=<?= $_GET['pg'] ?>" method="POST">
