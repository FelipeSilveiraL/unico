...
if(($rowDep = oci_fetch_array($resultDep, OCI_BOTH)) != FALSE) { 
    ...
    if (($rowDep['EMPRESA_DEPARTAMENTO'] != NULL) || ($rowDep['APROVADORESRH'] != NULL) || ($rowDep['GESTORDIRETO'] != NULL)) {
        header('location: ../front/departamentoNF.php?pg='.$_GET['pg'].'&msn=17');//msn=5 Departamento já possui relação em outra tabela
    }else{
    ...
    header('location: ../front/departamentoNF.php?pg='.$_GET['pg'].'&msn=14');//msn=3 deletado com sucesso
